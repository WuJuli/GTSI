...
def display(df):
    ...
    plt.savefig("./output1/display - cp.jpg")

    # Let's make our correlation matrix fancier
    plt.cla()
    corr_matrix = df.corr()
    ...
    ax = sns.heatmap(corr_matrix,
                     ...
                     fmt=".2f",
                     cmap='YlGnBu')
    fig.savefig("./output1/display - corr.jpg")
    return
...
def decisionTree(x_train, x_test, y_train, y_test):
    ...
    DT_randomsearched = DecisionTreeClassifier(min_samples_leaf=6, min_samples_split=3, random_state=322)
    ...
    plt.ylabel('Score')
    plt.ylim([-0.05, 1.05])
    plt.savefig("./output1/DT - MaxDepth.jpg")
    plt.cla()

    # Vary the min_samples_splits parameter from 1 to 15
    min_samples_split = range(2, 15)
    # random_search decision tree results
    DT_maxdepth_tuned = DecisionTreeClassifier(max_depth=3, min_samples_leaf=6, min_samples_split=3,
                                               random_state=322)
    # Calculate the training and testing scores
    ...
    plt.ylabel('Score')
    plt.ylim([-0.05, 1.05])
    plt.savefig("./output1/DT - MinSSplits.jpg")
    plt.cla()

    # final dt
    dt_final = DecisionTreeClassifier(max_depth=3, criterion='entropy', min_samples_leaf=6, min_samples_split=3,
    ...
    plt.savefig("./output1/DT - final.jpg")
    plt.cla()
    ...
    plt.savefig("./output1/DT - learning curve.jpg")
    plt.cla()
    return
...
def nnetwork(x_train, x_test, y_train, y_test):
    ...
    # f.savefig("./output1/NN - Accuracy&Loss.jpg")
    ...
    # Adding the input layer and the first hidden layer
    classifier2.add(Dense(units=6, kernel_initializer='uniform', activation='relu', input_dim=13))
    ...
    # Fitting the ANN to the Training set
    history2 = classifier2.fit(X_train, y_train, validation_data=(X_test, y_test), batch_size=100, epochs=1150)
    f, (ax1, ax2) = plt.subplots(1, 2, figsize=(20, 7))
    ...
    ax2.set_title('Loss')
    l2 = ax2.legend(loc="best")
    f.savefig("./output1/NN - Accuracy&Loss2.jpg")
    return
...
def boosting(x_train, x_test, y_train, y_test):
    ...
    print("defalut model : ", GBC.score(x_test, y_test))
    ...
    # Plot the validation curve
    plt.figure(figsize=(7, 5))
    plt.title('Validation Curve with GBC on heart disease dataset')
    plt.plot(max_depth, train_mean, 'o-', color='r', label='Training Score')
    plt.plot(max_depth, test_mean, 'o-', color='g', label='Validation Score')
    plt.fill_between(max_depth, train_mean - train_std, train_mean + train_std, alpha=0.15, color='r')
    ...
    plt.ylabel('Score')
    plt.ylim([-0.05, 1.05])
    plt.savefig("./output1/GBC - MaxDepth.jpg")
    plt.cla()
    ...
    plt.savefig("./output1/GBC - learning curve.jpg")
    plt.cla()
    return
...
def SVM(x_train, x_test, y_train, y_test):
    ...
    plt.xlabel("Kernals")
    plt.ylabel("Accuracy")
    plt.title("SVM performance of different kernels on heart disease dataset")
    plt.savefig("./output1/SVM - kernals.jpg")
    plt.cla()
    ...
    my_best = SVC(C=0.1, gamma=1)
    my_best.fit(x_train, y_train)
    print(my_best.score(x_test, y_test))

    # different gamma
    param_range = np.logspace(-6, -0, 6)
    SVC_best = SVC(C=0.1, gamma=1, random_state=322)
    train_scores, test_scores = validation_curve(
        SVC(C=0.1), x_train, y_train, param_name="gamma", param_range=param_range, scoring="accuracy")
    train_scores_mean = np.mean(train_scores, axis=1)
    train_scores_std = np.std(train_scores, axis=1)
    test_scores_mean = np.mean(test_scores, axis=1)
    test_scores_std = np.std(test_scores, axis=1)

    plt.title("Validation Curve with SVM on heart disease dataset")
    plt.xlabel(r"$\gamma$")
    plt.ylabel("Score")
    plt.ylim(0.0, 1.1)
    lw = 2
    ...
    plt.fill_between(param_range, test_scores_mean - test_scores_std,
                     ...
                     color="navy", lw=lw)
    plt.legend(loc="best")
    plt.savefig("./output1/SVM - gamma.jpg")
    plt.cla()
    ...
    plt.xlabel("Training examples")
    plt.ylabel("Score")
    plt.title("Support Vector Classifier on heart disease data")
    plt.savefig("./output1/SVM - learning curve1.jpg")
    plt.cla()
    # learning curves
    fig, axes = plt.subplots(3, 2, figsize=(10, 15))
    ...
    plt.savefig("./output1/SVM - learning curve2.jpg")
    plt.show()
    return
...
def knn(x_train, x_test, y_train, y_test):
    ...
    plt.xlabel("Training examples")
    plt.ylabel("Score")
    plt.title("kNN Classifier on heart disease data")
    plt.savefig("./output1/KNN - learning curve1.jpg")
    plt.cla()
    ...
    plot_learning_curve(KNeighborsClassifier(n_neighbors=14), title, x_train, y_train, axes=axes[:, 1], ylim=(0.0, 1.01),
                        cv=cv)
    plt.savefig("./output1/KNN - learning curve2.jpg")
    plt.cla()
    return
...
if __name__ == '__main__':
    # load the data
    data = pd.read_csv("input/heart_cleveland_upload.csv")
